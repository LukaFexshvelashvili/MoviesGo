...
if(isset($_SESSION['bookmarks']) && is_array($_SESSION['bookmarks'])){
 $new_bookmarks = json_decode($data['bookmarks']); // Decoding the new bookmarks
...
if(isset($_SESSION['watch_history']) && is_array($_SESSION['watch_history'])){
    $new_watch_history = json_decode($data['watch_history']); 
    $_SESSION['watch_history'] = array_unique(array_merge($_SESSION['watch_history'], $new_watch_history));
   }else{
       $_SESSION['watch_history'] = json_decode($data['watch_history']);
   }
$_SESSION['watch_history'] = json_decode($data['watch_history']);
$_SESSION['ui_settings'] = json_decode($data['ui_settings']);
$_SESSION['notifications'] = json_decode($data['notifications']);
