...
const n = document.querySelector(".mg_error_block"),
  ...
  P = document.querySelector(".mg_settings_block"),
  A = document.querySelectorAll(".mg_settings_column"),
...
let K = MG_PLAYER.seasons ? Object.keys(MG_PLAYER.seasons)[0] : 1,
  Q = 1;
var ee = null;
function InitializeVideo(e, t) {
  if (e.includes(".m3u8"))
    if (Hls.isSupported()) {
      ee && ee.destroy();
      var s = new Hls();
      s.loadSource(e),
        s.attachMedia(i),
        s.on(Hls.Events.MANIFEST_PARSED, function () {
          null !== t &&
            (0 == t.isPaused
              ? i.play()["catch"]((e) => console.warn("Autoplay blocked:", e))
              : playPause("pause"));
        });
    } else
      i.canPlayType("application/vnd.apple.mpegurl")
        ? changeVideoUrl(e)
        : (n.classList.remove("mg_error_hidden"), setLoading(!1));
  else changeVideoUrl(e);
  i.playbackRate = ne.speed;
}
let ne;
...
localStorage.getItem("mg_player_controls")
  ? ((ne = JSON.parse(localStorage.getItem("mg_player_controls"))),
    (O.innerText = ne.lang ? ne.lang.toString() : "GEO"),
    (G.innerText = ne.quality ? ne.quality.toString() : "HD"),
    (F.innerText = ne.speed ? ne.speed.toString() : "1"))
  : ((ne = { lang: "GEO", volume: 1, speed: 1, quality: "HD" }),
    localStorage.setItem("mg_player_controls", JSON.stringify(ne)));
let te = !1,
  se = !1,
  ie = !1,
  oe = !0;
...
!(function initializePlayer() {
  0 == ne.volume && soundOnOff(),
    "SERIES" == MG_PLAYER.type && Z.classList.remove("mg_player_eps_hidden"),
    (function getCheckOfControls() {
      (ne.lang = MG_PLAYER.languages[ne.lang] ? ne.lang : "GEO"),
        (ne.quality = MG_PLAYER.languages[ne.lang]?.[ne.quality]
          ? ne.quality
          : "HD");
      let e =
        MG_PLAYER.languages[ne.lang]?.[ne.quality] ||
        MG_PLAYER.languages.GEO?.[ne.quality] ||
        MG_PLAYER.languages.GEO?.HD;
      q.forEach((e) => {
        e.innerHTML.trim() == ne.speed &&
      ...
      if (localStorage.getItem("mg_player")) {
        ...
        0 !== t.length
          ? (!(function movetoFirstItem(e, n) {
            ...
            localStorage.setItem("mg_player", JSON.stringify(n)))
          : ("SERIES" == MG_PLAYER.type
              ? n.unshift({ id: MG_PLAYER.id, time: 0, episode: 1, season: K })
        ...
        "SERIES" == MG_PLAYER.type
          ? localStorage.setItem(
              "mg_player",
              JSON.stringify([
                { id: MG_PLAYER.id, time: 0, episode: 1, season: K },
              ])
            )
          : localStorage.setItem(
              "mg_player",
  ...
  for (const [e] of Object.entries(MG_PLAYER.languages[ne.lang]))
    $.innerHTML += `<div class='mg_button${
      ne.quality == e ? " mg_button_active" : ""
    }'>${e}</div>`;
  for (const [e] of Object.entries(MG_PLAYER.languages))
    D.innerHTML += `<div class='mg_button${
      ne.lang == e ? " mg_button_active" : ""
    }'>${e}</div>`;
})(),
  ...
  e.addEventListener("click", function playerClick(e) {
    mouseMoving(),
      P.classList.contains("mg_settings_hidden") ||
        M.contains(e.target) ||
        closeSettings();
  }),
...
const de = Array.from($.children),
  ue = Array.from(D.children);
...
function closeSettings() {
  P.classList.add("mg_settings_hidden"),
    setTimeout(() => {
      A.forEach((e) => e.classList.add("mg_settings_column_hide")),
        A[0].classList.remove("mg_settings_column_hide");
    }, 200);
}
...
function soundOnOff() {
  i.volume > 0
    ...
    : (changeSounds("on"),
      ne.volume > 0.08
        ? ((i.volume = ne.volume),
          (g.value = 100 * ne.volume),
          saveControls({ volume: ne.volume }))
...
de.forEach((e) => {
  e.addEventListener("click", () => {
    if (!e.classList.contains("mg_button_active")) {
      var n = i.currentTime,
        t = i.paused;
      InitializeVideo(MG_PLAYER.languages[ne.lang][e.innerText], {
        isPaused: t,
      }),
  ...
  ue.forEach((e) => {
    e.addEventListener("click", () => {
      if (!e.classList.contains("mg_button_active")) {
        var n = i.currentTime,
          t = i.paused;
        InitializeVideo(MG_PLAYER.languages[e.innerText][ne.quality], {
          isPaused: t,
        }),
  ...
  I.forEach((e) => {
    e.addEventListener("click", () => {
      A.forEach((e) => e.classList.add("mg_settings_column_hide")),
        A[parseInt(e.getAttribute("data-col-index"))].classList.remove(
...
function printEpisodes() {
  ...
  (N.innerHTML = ""),
    MG_PLAYER.seasons[K].forEach((n, t) => {
      (t += 1),
        (N.innerHTML += `<div data-ep="${t}" class="mg_ep_button ${
          Q == t && e == K ? "mg_ep_button_active" : ""
        } ">\n      ${t} ეპიზოდი\n      </div>`);
    });
  let t = Array.from(N.children);
  t.forEach((e) => {
    e.addEventListener("click", () => {
      (Q = e.getAttribute("data-ep")),
        n.classList.add("mg_error_hidden"),
        (function changeEpisode(e) {
          if (
            ...
            let e = localStorage.getItem("mg_player"),
              n = JSON.parse(e);
            (n[0].time = 0),
              (n[0].episode = parseInt(Q)),
              (n[0].season = K),
              localStorage.setItem("mg_player", JSON.stringify(n));
          }
          (MG_PLAYER.languages = (function getEpisodesObjectRequest() {
            return MG_PLAYER.seasons[K][Q - 1].languages;
          })()),
            0 == se && firstStart();
...
function getEpisodeRequest() {
  return MG_PLAYER.seasons[K][Q - 1].languages[ne.lang]
    ? MG_PLAYER.seasons[K][Q - 1].languages[ne.lang][ne.quality]
      ? MG_PLAYER.seasons[K][Q - 1].languages[ne.lang][ne.quality]
      : Object.values(MG_PLAYER.seasons[K][Q - 1].languages[ne.lang])[0]
    : Object.values(Object.values(MG_PLAYER.seasons[K][Q - 1].languages)[0])[0];
}
