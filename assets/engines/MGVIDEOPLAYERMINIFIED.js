...
const n = document.querySelector(".mg_error_block"),
  ...
  h = document.querySelector(".mg_time_indicator_helper"),
  y = document.querySelector(".mg_timeline_helper"),
  L = document.querySelector(".mg_helper_video"),
  E = document.querySelector(".mg_timeline_helper_time"),
  C = document.querySelector(".mg_starttime"),
  S = document.querySelector(".mg_endtime"),
  T = document.querySelector(".mg_skip_left_button"),
  w = document.querySelector(".mg_skip_right_button"),
  b = document.querySelector(".mg_fullscreen"),
  k = document.querySelector("#mg_fullscreen_on_"),
  M = document.querySelector("#mg_fullscreen_off_"),
  q = document.querySelectorAll(".mg_speed_button"),
  x = document.querySelector(".mg_controls"),
  R = document.querySelector(".mg_download"),
  A = document.querySelector(".mg_frame"),
  H = document.querySelector(".mg_settings_block"),
  P = document.querySelectorAll(".mg_settings_column"),
  O = document.querySelectorAll(".mg_settings_back"),
  I = document.querySelectorAll(".mg_setting_choose"),
  G = document.querySelector("#active_setting_language"),
  Y = document.querySelector("#active_setting_quality"),
  V = document.querySelector("#active_setting_speed"),
  F = document.querySelector("#mg_settings_toggler"),
  z = document.querySelector(".mg_skip_left"),
  D = document.querySelector(".mg_skip_right"),
  $ = document.querySelector(".mg_languages"),
  B = document.querySelector(".mg_qualities"),
  N = document.querySelector(".mg_context_menu"),
  J = document.querySelector(".mg_player_eps_scroll"),
  Z = document.querySelector(".mg_player_seasons"),
  j = document.querySelector(".mg_player_eps"),
  X = document.querySelector(".mg_player_eps_container"),
  W = document.querySelector(".mg_player_ep_button"),
  U = document.querySelector(".mg_eps_closer");
let K = !1;
e.addEventListener("click", () => {
  K = !0;
}),
  document.addEventListener("click", (n) => {
    e.contains(n.target) || (K = !1);
  });
let Q,
  ee = MG_PLAYER.seasons ? Object.keys(MG_PLAYER.seasons)[0] : 1,
  ne = 1;
function InitializeVideo(e) {
    ...
    if (Hls.isSupported()) {
      ...
      t.loadSource(e),
        ...
        t.on(Hls.Events.ERROR, function () {
          n.classList.remove("mg_error_block_hidden");
        });
    } else
  ...
  i.playbackRate = Q.speed;
}
function changeVideoUrl(e) {
  (i.src = e), (L.src = e);
}
localStorage.getItem("mg_player_controls")
  ? ((Q = JSON.parse(localStorage.getItem("mg_player_controls"))),
    (G.innerText = Q.lang ? Q.lang.toString() : "GEO"),
    (Y.innerText = Q.quality ? Q.quality.toString() : "HD"),
    (V.innerText = Q.speed ? Q.speed.toString() : "1"))
  : ((Q = { lang: "GEO", volume: 1, speed: 1, quality: "HD" }),
    localStorage.setItem("mg_player_controls", JSON.stringify(Q)));
let te = !1,
  se = !1,
  ie = !1,
  oe = !0;
...
!(function initializePlayer() {
  0 == Q.volume && soundOnOff(),
    "SERIES" == MG_PLAYER.type && j.classList.remove("mg_player_eps_hidden"),
    (function getCheckOfControls() {
      (Q.lang = MG_PLAYER.languages[Q.lang] ? Q.lang : "GEO"),
        (Q.quality = MG_PLAYER.languages[Q.lang]?.[Q.quality]
          ? Q.quality
          : "HD");
      let e =
        MG_PLAYER.languages[Q.lang]?.[Q.quality] ||
        MG_PLAYER.languages.GEO?.[Q.quality] ||
        MG_PLAYER.languages.GEO?.HD;
      q.forEach((e) => {
        e.innerHTML.trim() == Q.speed &&
      ...
      if (localStorage.getItem("mg_player")) {
        ...
        0 !== t.length
          ? (!(function movetoFirstItem(e, n) {
            ...
            "SERIES" == MG_PLAYER.type &&
              ...
              t[0].episode <= MG_PLAYER.seasons[t[0].season].length
                ? ((ee = t[0].season),
                  (ne = t[0].episode),
                  setTimeout(() => {
                    const e = Z.querySelector(
                        `[data-season="${t[0]?.season}"]`
                      ),
                      n = J.querySelector(`[data-ep="${t[0]?.episode}"]`);
                    e && Z && (Z.scrollTop = e.offsetTop - Z.offsetTop - 100),
                      n && J && (J.scrollTop = n.offsetTop - J.offsetTop - 100);
                  }, 0),
            ...
            localStorage.setItem("mg_player", JSON.stringify(n)))
          : ("SERIES" == MG_PLAYER.type
              ? n.unshift({ id: MG_PLAYER.id, time: 0, episode: 1, season: ee })
        ...
        "SERIES" == MG_PLAYER.type
          ? localStorage.setItem(
              "mg_player",
              JSON.stringify([
                { id: MG_PLAYER.id, time: 0, episode: 1, season: ee },
              ])
            )
          : localStorage.setItem(
              "mg_player",
  ...
  for (const [e] of Object.entries(MG_PLAYER.languages[Q.lang]))
    B.innerHTML += `<div class='mg_button${
      Q.quality == e ? " mg_button_active" : ""
    }'>${e}</div>`;
  for (const [e] of Object.entries(MG_PLAYER.languages))
    $.innerHTML += `<div class='mg_button${
      Q.lang == e ? " mg_button_active" : ""
    }'>${e}</div>`;
})(),
  ...
  e.addEventListener("click", function playerClick(e) {
    mouseMoving(),
      H.classList.contains("mg_settings_hidden") ||
        x.contains(e.target) ||
        closeSettings();
  }),
  W.addEventListener("click", function openEpisodes() {
    X.classList.add("mg_player_eps_container_show"),
      W.classList.add("mg_player_ep_button_hide");
  }),
  U.addEventListener("click", closeEpisodes),
  e.addEventListener("mousemove", mouseMoving),
  e.addEventListener("touchmove", mouseMoving, { passive: !0 }),
  b.addEventListener("click", fullscreenOnOff),
  o.addEventListener("click", firstStart),
  z.addEventListener("touchend", function skipLeftDbl(e) {
    e.preventDefault(),
      dbClick(
        () => {
          i.currentTime -= 10;
      ...
      (C.innerHTML = formatTime(i.currentTime)),
      measureTime(i.currentTime);
  }),
  D.addEventListener("touchend", function skipRightDbl(e) {
    e.preventDefault(),
      dbClick(
        () => {
          i.currentTime += 10;
  ...
  T.addEventListener("click", function skipLeft() {
    (i.currentTime -= 10),
      (C.innerHTML = formatTime(i.currentTime)),
      measureTime(i.currentTime);
  }),
  w.addEventListener("click", function skipRight() {
    (i.currentTime += 10),
      (C.innerHTML = formatTime(i.currentTime)),
      measureTime(i.currentTime);
  }),
  ...
  a.addEventListener("touchend", showhideControls, { passive: !0 }),
  r.forEach((e) => {
  ...
  g.addEventListener("input", function measureSound() {
    0 == g.value ? changeSounds("off") : changeSounds("on");
    (Q.volume = g.value / 100),
      saveControls({ volume: g.value / 100 }),
      (i.volume = g.value / 100);
  }),
  F.addEventListener("click", function toggleSettings() {
    H.classList.toggle("mg_settings_hidden");
  }),
  A.addEventListener("click", async function togglePIP() {
    document.pictureInPictureElement
      ? await document.exitPictureInPicture()
      : await i.requestPictureInPicture();
  }),
  R.addEventListener("click", function downloadMovie() {
  ...
  p.addEventListener("mousemove", function seeTime(e) {
    if (ie) {
      var n = (100 / p.offsetWidth) * e.offsetX;
      (y.style.opacity = 1),
        (y.style.transform = `translateX(${e.offsetX - y.offsetWidth / 2}px)`);
      var t = percentageToTime(n);
      t &&
        ((E.innerHTML = formatTime(t)),
        (L.currentTime = t.toFixed(6)),
        (h.style.width = n + "%"));
    }
  }),
  p.addEventListener(
    ...
    function seeTimeTouch(e) {
      ...
      if (ie) {
        ...
        (y.style.opacity = 1),
          (y.style.transform = `translateX(${
            n.clientX - p.getBoundingClientRect().left - y.offsetWidth / 2
          }px)`);
  ...
  document.addEventListener("keydown", function handleKeyPress(e) {
    ...
    switch (e.key) {
      case "ArrowLeft":
        (i.currentTime -= 10), (C.innerHTML = formatTime(i.currentTime));
        break;
      case "ArrowRight":
        (i.currentTime += 10), (C.innerHTML = formatTime(i.currentTime));
        break;
  ...
  i.addEventListener("playing", () => {
    n.classList.add("mg_error_block_hidden"), setLoading(!1);
  }),
  i.addEventListener("canplaythrough", () => {
    setLoading(!1);
  }),
  i.addEventListener("loadeddata", function () {
    n.classList.add("mg_error_block_hidden");
    ...
    (i.currentTime = e.toFixed(6)),
      ...
      setLoading(!1),
      (C.innerHTML = formatTime(i.currentTime)),
      (S.innerHTML = formatTime(i.duration));
  });
let le,
  ae = 0;
...
function closeEpisodes() {
  X.classList.remove("mg_player_eps_container_show"),
    W.classList.remove("mg_player_ep_button_hide");
}
...
!(function mouseTouchDragger() {
  p.addEventListener("mousedown", function () {
    (re = !0), (oe = !1), document.addEventListener("mousemove", onDraging);
  }),
...
const de = Array.from(B.children),
  ue = Array.from($.children);
function hideControls() {
  x.classList.add("mg_controls_hidden"),
    X.classList.contains("mg_player_eps_container_show") ||
      (j.classList.add("mg_controls_hidden"), closeEpisodes()),
    e.classList.add("mg_hide_cursor"),
    c.classList.add("mg_controls_hidden"),
    closeSettings();
}
function showControls() {
  x.classList.remove("mg_controls_hidden"),
    j.classList.remove("mg_controls_hidden"),
    e.classList.remove("mg_hide_cursor"),
    c.classList.remove("mg_controls_hidden");
}
function closeSettings() {
  H.classList.add("mg_settings_hidden"),
    setTimeout(() => {
...
function removeSeeTime() {
  (y.style.opacity = 0), (h.style.width = "0%");
}
...
function soundOnOff() {
  i.volume > 0
    ...
    : (changeSounds("on"),
      Q.volume > 0.08
        ? ((i.volume = Q.volume),
          (g.value = 100 * Q.volume),
          saveControls({ volume: Q.volume }))
...
de.forEach((e) => {
  e.addEventListener("click", () => {
    if (!e.classList.contains("mg_button_active")) {
      var n = i.currentTime,
        t = i.paused;
      InitializeVideo(MG_PLAYER.languages[Q.lang][e.innerText]),
  ...
  ue.forEach((e) => {
    e.addEventListener("click", () => {
      if (!e.classList.contains("mg_button_active")) {
        var n = i.currentTime,
          t = i.paused;
        InitializeVideo(MG_PLAYER.languages[e.innerText][Q.quality]),
          saveControls({ lang: e.innerText }),
          (G.innerText = e.innerText),
          (i.currentTime = n.toFixed(6)),
          0 == t && i.play(),
          ue.forEach((e) => e.classList.remove("mg_button_active")),
          e.classList.add("mg_button_active");
      }
    });
  }),
...
function saveControls({ lang: e, volume: n, speed: t, quality: s }) {
  if (localStorage.getItem("mg_player_controls")) {
    ...
    (o.volume = n ?? o.volume),
      ...
      (o.speed = t ?? o.speed),
      (G.innerText = e ? o.lang.toString() : "GEO"),
      (Y.innerText = s ? o.quality.toString() : "HD"),
      (V.innerText = t ? o.speed.toString() : "1"),
...
function printEpisodes() {
  ...
  (J.innerHTML = ""),
    MG_PLAYER.seasons[ee].forEach((n, t) => {
      (t += 1),
        (J.innerHTML += `<div data-ep="${t}" class="mg_ep_button ${
          ne == t && e == ee ? "mg_ep_button_active" : ""
        } ">\n      ${t} ეპიზოდი\n      </div>`);
    });
  let t = Array.from(J.children);
  t.forEach((e) => {
    e.addEventListener("click", () => {
      (ne = e.getAttribute("data-ep")),
        n.classList.add("mg_error_block_hidden"),
        (function changeEpisode(e) {
          if (
            ...
            let e = localStorage.getItem("mg_player"),
              n = JSON.parse(e);
            (n[0].time = 0),
              (n[0].episode = parseInt(ne)),
              (n[0].season = ee),
              localStorage.setItem("mg_player", JSON.stringify(n));
          }
          (MG_PLAYER.languages = (function getEpisodesObjectRequest() {
            return MG_PLAYER.seasons[ee][ne - 1].languages;
          })()),
            0 == se && firstStart();
...
function getEpisodeRequest() {
  return MG_PLAYER.seasons[ee][ne - 1].languages[Q.lang]
    ? MG_PLAYER.seasons[ee][ne - 1].languages[Q.lang][Q.quality]
      ? MG_PLAYER.seasons[ee][ne - 1].languages[Q.lang][Q.quality]
      : Object.values(MG_PLAYER.seasons[ee][ne - 1].languages[Q.lang])[0]
    : Object.values(
        Object.values(MG_PLAYER.seasons[ee][ne - 1].languages)[0]
      )[0];
}
...
!(function initialiseEpisodes() {
  "SERIES" == MG_PLAYER.type &&
    (null == ee && null == ne && ((ee = 1), (ne = 1)),
    (function printSeasons() {
      Z.innerHTML = "";
      for (const [n] of Object.entries(MG_PLAYER.seasons))
        Z.innerHTML += `<div data-season="${n}" class="mg_se_button ${
          ee == n ? "mg_se_button_active" : ""
        } ">\n    ${n}\n  </div>`;
      let e = Array.from(Z.children);
      e.forEach((n) => {
        n.addEventListener("click", () => {
          (ee = n.getAttribute("data-season")), printEpisodes();
          const t = J.querySelector(`[data-ep="${ne}"]`),
            s = getCurStorage().season;
          (J.scrollTop = t && s == ee ? t.offsetTop - J.offsetTop - 100 : 0),
